document.getElementById("spam").onclick = function(){
  ...
  if (amo >= 101) {
    document.getElementById("status").innerHTML = "Status : Error จำนวนข้อความไม่เกิน 100";
} else if(amo == 0){
document.getElementById("status").innerHTML = "Status : Error ใส่จำนวนรอบมากกว่า 1 รอบ";
} else if(myn == 0) {
    document.getElementById("status").innerHTML = "Status : Error กรุณากรอกเบอร์เป้าหมาย";
} else {
  test()
  }
}
...
document.getElementById("done").innerHTML = "SPAM To Number Phone > "+myn;
  ...
  setTimeout(() => {
    document.getElementById("spam").innerHTML = "คลิกที่นี่เพื่อยิงเบอร์อีกครั้ง";
